...
class ListTournamentDarts extends Component
{
...
//        dd($this);
    ...
    public function startTournament($id)
    {
        ...
        foreach ($matchs as $match) {
            ...
            $ratioPlayer2 = (Elo::query()->where('user_id', $player1->id)->where('sport_id', 2)->first()->elo / Elo::query()->where('user_id', $player2->id)->where('sport_id', 2)->first()->elo) + 1;
            $ratioPlayer1 = (Elo::query()->where('user_id', $player2->id)->where('sport_id', 2)->first()->elo / Elo::query()->where('user_id', $player1->id)->where('sport_id', 2)->first()->elo) + 1;

            //Couleur blanche donnée au joueur ayant le elo le plus faible
            if (Elo::query()->where('user_id', $player1->id)->where('sport_id',2)->first()->elo >= Elo::query()->where('user_id', $player2->id)->where('sport_id',2)->first()->elo) {
                $colorPlayer1 = "noir";
                $colorPlayer2 = "blanc";
            } else {
                $colorPlayer1 = "blanc";
    ...
    {
        return view('livewire.darts.tournament.list-tournament-darts', [
