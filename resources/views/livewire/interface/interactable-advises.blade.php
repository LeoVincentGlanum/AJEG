    ...
    <div wire:poll="sync">
        @if($visible)
            <div class=" grid grid-cols-10  overflow-hidden rounded-md border border-gray-300   bg-grey-500">
                ...
                @if(isset($buttonText))
                    <a wire:click="refreshParent" class="col-span-2 inline-flex items-center rounded-md border border-transparent bg-blue-600 m-2 px-6 py-3 text-base font-medium text-white shadow-sm hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">
                        {{ __($buttonText) }}
